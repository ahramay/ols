...
class PricePlanTable extends Component {
  componentDidMount = () => {
    const { token, subscriptionPlan } = this.props;
    $("#subscriptionPlanTable").DataTable({
      ...
      order: [[0, "_id"]],
      ajax: {
        url: apiPath + "/price_plans/data_table/" + subscriptionPlan,
        type: "GET",
...
export default connect(mapStateToProps, mapDispatchToProps)(PricePlanTable);
